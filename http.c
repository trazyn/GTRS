...
static FILE *ropen(const char *host, unsigned short port, int timeout)
{
	...
	{
		fprintf(stderr, "failed to get host(%s) info\n", host);
	...
	if(0 == ret)
	{
		fprintf(stderr, "connect to '%s' timeout\n", host);
		return NULL;
	}

	if(ret < 0 && EINPROGRESS != errno)
	{
		fprintf(stderr, "failed to connect '%s': %s", host, strerror(errno));
		return NULL;
	}
...
char **fetch(const char *url, FILE **handle, const char *post, const char *type)
{
	...
	if(200 != nstatus && 301 != nstatus && 302 != nstatus)
	{
		...
		fprintf(stderr, "error response status: %s\n", status);
